...
def chat(data: dict):
    ...
    if recherche:
        ...
        if contexte:
            system += "\n\nUtilise ces résultats de recherche récents pour répondre :\n" + contexte
    ...
    return {"reply": response.choices[0].message.content, "recherche": recherche}
...
def generer_word(data: dict):
    sujet = data.get("sujet", "")
    contenu = demander(f"Rédige un document complet et structuré sur : {sujet}. Utilise des titres, sous-titres et paragraphes.")
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=nexa_document.docx"}
    )

@app.post("/excel")
def generer_excel(data: dict):
    sujet = data.get("sujet", "")
    contenu = demander(f"Crée un tableau de données sur : {sujet}. Format CSV simple avec en-têtes et lignes séparées par des virgules. Ne mets AUCUN autre texte.")
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=nexa_tableau.xlsx"}
    )

@app.post("/pptx")
def generer_pptx(data: dict):
    sujet = data.get("sujet", "")
    contenu = demander(f"""Crée une présentation de 5-7 slides sur : {sujet}.
...
(etc.)""")
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=nexa_presentation.pptx"}
    )
...
def generer_pdf(data: dict):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=nexa_document.pdf"}
    )
...
async def analyser(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {"reply": analyse, "contenu_extrait": texte[:500]}
    except Exception as e:
